...
async function testContextCommands() {
    ...
    // Clean up test data
    const notesFile = path.join(__dirname, '../notes.quick-test.json');
    if (fs.existsSync(notesFile)) {
        fs.writeFileSync(notesFile, '{"notes": [], "latestNoteId": 0}');
        console.log('Cleaned up notes.quick-test.json');
    }
    
    const port = 30000 + Math.floor(Math.random() * 1000);
    const serverProcess = spawn('node', [path.join(__dirname, '../min.js')], {
        env: { ...process.env, NODE_PORT: port.toString(), NODE_ENV: 'test', TEST_SUITE: 'quick-test' },
        stdio: 'inherit'
    });
